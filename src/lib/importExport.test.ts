...
describe("import/export", () => {
  it("round-trips a MAPPING document", () => {
    const original = setCardRoles(brandedDemo, "main", 62962630, [
      "starter",
      "extender",
    ]);
  ...
  it("keeps multi-role labels through serialization", () => {
    ...
    expect(JSON.parse(json).main[0].roles).toEqual(["starter", "extender"]);
  });
  ...
  it("keeps quantities through serialization", () => {
    ...
    doc = {
      ...doc,
      main: [{ card_id: 7, quantity: 1, roles: [] }],
    };
  ...
  it("keeps main, extra, and side distinct", () => {
    const doc = parseMappingJson(
      serializeMapping({
        ...
        main: [{ card_id: 1, quantity: 3, roles: ["starter"] }],
        extra: [{ card_id: 2, quantity: 1, roles: ["extender"] }],
        side: [{ card_id: 3, quantity: 2, roles: ["interaction"] }],
      }),
    );
  ...
  it("emits the YAPPING archetype fields load_archetype expects", () => {
    ...
    expect(exported.card_roles["55273560"]).toEqual([
      "starter",
      "extender",
      "recovery",
    ]);
  ...
  it("coerces a blank name instead of dropping the document", () => {
    const parsed = parseMappingJson(
      JSON.stringify({
        schema_version: 1,
        name: "   ",
        main: [{ card_id: 1, quantity: 1, roles: ["starter"] }],
        extra: [],
        side: [],
  ...
  it("collapses duplicate card ids in a section", () => {
    const parsed = parseMappingJson(
      JSON.stringify({
        ...
        name: "dup",
        main: [
          { card_id: 1, quantity: 2, roles: ["starter"] },
          { card_id: 1, quantity: 1, roles: ["extender"] },
        ],
        extra: [],
        side: [],
        analysis: { opening_hand_size: 5 },
    ...
    expect(parsed.main).toEqual([
      { card_id: 1, quantity: 3, roles: ["starter", "extender"] },
    ]);
  });
